 ...
 *
 *  Interact with the Javalin controller by:
 *  a) for GET requests, using the CURL command in the terminal (eg curl localhost:8080/books). Posting is
 *     trickier to format with CURL: https://linuxize.com/post/curl-post-request/
 *  b) If working on a local machine and not in a browser-based IDE, navigating to an endpoint in the web browser
 *     (eg localhost:8080/books) will perform a GET request.
 *  c) If working on a local machine and not in a browser-based IDE, using the desktop version of Postman
